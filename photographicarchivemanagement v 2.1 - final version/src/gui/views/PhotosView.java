...
public class PhotosView extends JPanel {
    ...
    private void initUI() {
        ...
        searchField.setToolTipText("Search by ID or Subject…");
        searchField.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(Theme.BORDER_COLOR),
                new EmptyBorder(6, 10, 6, 10)));
        searchField.setPreferredSize(new Dimension(180, 36));
        searchField.addActionListener(this::performSearch); // Enable Enter key search
    ...
    private void performSearch(ActionEvent e) {
        ...
        // If empty query, just reload the filtered view
        if (q.isEmpty()) {
            loadData(selectedArchive);
            return;
        }
        ...
        tableModel.setRowCount(0);
        Collection<Archivio> archivi;

        if (selectedArchive == null || selectedArchive.equals("All Archives")) {
            archivi = GestoreArchivi.getInstance().getArchivi().values();
        } else {
            Archivio a = GestoreArchivi.getInstance().getArchivio(selectedArchive);
            archivi = (a != null) ? Collections.singletonList(a) : Collections.emptyList();
        }

        for (Archivio a : archivi) {
            for (Fotografia f : a.getFotografie()) {
                String id = f.getIdFoto().toLowerCase();
                String size = f.getDimensione().toLowerCase();
                String condition = f.getStatoConservazione().toLowerCase();
                String subjKey = (f.getSoggetto() != null) ? f.getSoggetto().getKey().toLowerCase() : "";
                String subjDesc = (f.getSoggetto() != null) ? f.getSoggetto().getDescription().toLowerCase() : "";
                String type = (f instanceof FotoAColore) ? "color" : "b/w";
                String printType = (f instanceof FotoAColore) ? ((FotoAColore) f).getTipoStampa().toLowerCase() : "—";
                String archiveName = a.getNomeArchivio().toLowerCase();

                boolean match = id.contains(q) || size.contains(q) || condition.contains(q) ||
                        subjKey.contains(q) || subjDesc.contains(q) ||
                        type.contains(q) || printType.contains(q) || archiveName.contains(q);
