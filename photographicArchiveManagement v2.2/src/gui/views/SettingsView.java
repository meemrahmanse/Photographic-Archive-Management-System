...
public class SettingsView extends JPanel {
    ...
    private void initUI() {
        ...
        scrollableContent.add(makeSectionLabel("Data Management"));
        scrollableContent.add(Box.createVerticalStrut(12));
        scrollableContent.add(makeSettingCard(
                "Save All Data",
                "Persist all archives and subjects to their respective JSON files on disk.",
                "Save to JSON", Theme.ACCENT_COLOR, Color.WHITE,
                e -> {
                    try {
                        GestoreArchivi.getInstance().salvaSuFile();
                        CatalogoSoggetti.getInstance().salvaSuFile();
                        JOptionPane.showMessageDialog(this, "Data saved successfully to JSON files.", "Saved",
                                JOptionPane.INFORMATION_MESSAGE);
                    } catch (Exception ex) {
                        JOptionPane.showMessageDialog(this, "Error: " + ex.getMessage(), "Save Error",
        ...
        // Section: Demo Data
        scrollableContent.add(makeSectionLabel("Developer Tools"));
        scrollableContent.add(Box.createVerticalStrut(12));
        scrollableContent.add(makeSettingCard(
                "Load Sample Subjects",
                "Populate the subjects catalog with representative demo data (Persons, Politicians, Artists, Places, Objects).",
                "Load Samples", new Color(109, 40, 217), Color.WHITE,
                e -> {
                    try {
                        CatalogoSoggetti.getInstance()
                                .aggiungiSoggetto(new Personaggio("p1", "Mario Rossi", 'M', false, 1980));
                        CatalogoSoggetti.getInstance().aggiungiSoggetto(
                                new Politico("p2", "Luigi Verdi", 'M', false, 1970, "Centro", "Ministro"));
                        CatalogoSoggetti.getInstance()
                        ...
                        JOptionPane.showMessageDialog(this, "Sample subjects loaded. Go to Subjects Catalog to view.",
                                "Done", JOptionPane.INFORMATION_MESSAGE);
                    } catch (IllegalArgumentException ex) {
                        JOptionPane.showMessageDialog(this, "Sample data already exists.", "Warning",
